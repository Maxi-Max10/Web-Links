 ...
 if (isset($_POST["ajaxR"]))
{
    ...
    if ($usuario == "") {
        ...
        $consulta = $bd->query("SELECT password_us FROM usuarios WHERE email = '".$email."'");
        $contrasenia = $consulta->fetchAll(PDO::FETCH_OBJ);
        //print $contasenia;

        foreach ($contrasenia as $dato){
            $contra = $dato->password_us;
        }

        mail($email,"Recuperación de contraseña","Contraseña","USUARIO ".$usuario." SU CONTRASEÑA ES ".$contra);
